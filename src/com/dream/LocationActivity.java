...
public class LocationActivity extends Activity implements LocationSource,
	...
	private void showMore() {
		if (query != null && poiSearch != null && poiResult != null) {
			if (poiResult.getPageCount() - 1 > currentPage) {
				...
				Toast.makeText(LocationActivity.this, "对不起，没有更多位置信息了", Toast.LENGTH_SHORT).show();
			}
		}
	}
	...
	private void showSuggestCity(List<SuggestionCity> cities) {
		...
		Toast.makeText(LocationActivity.this, infomation, Toast.LENGTH_SHORT).show();
	}
		...
		if (mListener != null && aLocation != null) {
			...
			if (!hasGot) {
				...
				if ("".equals(keyWord)) {
					...
					Toast.makeText(LocationActivity.this, "对不起，查询不到您的信息", Toast.LENGTH_SHORT).show();
					return;
				} else {
					doSearchQuery();
				}
	...
	public void onPoiSearched(PoiResult result, int rCode) {
		if (rCode == 0) {
			...
			Toast.makeText(LocationActivity.this, "搜索失败,请检查网络连接！", Toast.LENGTH_SHORT).show();
		} else if (rCode == 32) {
			Toast.makeText(LocationActivity.this, "key验证无效！", Toast.LENGTH_SHORT).show();
		} else {
			
			Toast.makeText(LocationActivity.this, "未知错误，请稍后重试！", Toast.LENGTH_SHORT).show();
		}
	}
